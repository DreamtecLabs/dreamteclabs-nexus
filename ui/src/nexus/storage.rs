...
fn usage_percent(used: u64, total: u64) -> f64 {
    if total == 0 { 0.0 } else { (used as f64 / total as f64 * 100.0).clamp(0.0, 100.0) }
}

fn normalized_status(status: &str) -> String { status.trim().to_ascii_lowercase() }
...
fn merge_shared_storage(existing: &mut StorageEntry, used: u64, total: u64, status: &str) {
    ...
    if matches!(incoming_status.as_str(), "available" | "online") || !matches!(current_status.as_str(), "available" | "online") {
...
fn split_storage(remotes: &[RemoteResources]) -> (Vec<StorageEntry>, Vec<StorageEntry>, Vec<String>) {
    ...
    for remote in remotes {
        if let Some(error) = &remote.error { errors.push(format!("{}: {}", remote.remote, error)); }
        for resource in &remote.resources {
            match resource {
                Resource::PveStorage(storage) => {
                    let key = if storage.shared { format!("{}/shared/{}", remote.remote, storage.storage) } else { format!("{}/{}/{}", remote.remote, storage.node, storage.storage) };
                    if storage.shared {
                        pve.entry(key).and_modify(|entry| merge_shared_storage(entry, storage.disk, storage.maxdisk, &storage.status)).or_insert_with(|| StorageEntry {
                            remote: remote.remote.clone(), name: storage.storage.clone(), location: "Shared cluster storage".to_string(), kind: "Shared".to_string(), status: storage.status.clone(), used: storage.disk, total: storage.maxdisk,
                        });
                    } else {
                        pve.insert(key, StorageEntry { remote: remote.remote.clone(), name: storage.storage.clone(), location: storage.node.clone(), kind: "Local".to_string(), status: storage.status.clone(), used: storage.disk, total: storage.maxdisk });
                    }
                }
                Resource::PbsDatastore(datastore) => {
                    let key = format!("{}/{}", remote.remote, datastore.name);
                    pbs.insert(key, StorageEntry {
                        remote: remote.remote.clone(), name: datastore.name.clone(), location: datastore.backing_device.clone().unwrap_or_else(|| "PBS datastore".to_string()), kind: datastore.backend_type.as_ref().map(ToString::to_string).unwrap_or_else(|| "Datastore".to_string()), status: if datastore.maintenance.is_some() { "maintenance".to_string() } else { "online".to_string() }, used: datastore.disk, total: datastore.maxdisk,
                    });
                }
                _ => {}
            }
...
fn tier_summary(title: &str, icon: &str, entries: &[StorageEntry], tone: &str) -> Html {
    ...
    let is_pve = tone == "pve";
    let capacity_label = if is_pve { "Configured capacity" } else { "Capacity" };
    let used_label = if is_pve { "Reported used" } else { "Used" };
    let free_label = if is_pve { "Reported free" } else { "Free" };
    let footer = if is_pve { format!("{pct:.1}% target utilization · physical backing may overlap") } else { format!("{pct:.1}% utilized") };
    html! {
...
fn storage_table(title: &str, subtitle: &str, icon: &str, entries: &[StorageEntry]) -> Html {
    html! {
        <section class="nexus-storage-panel">
            ...
            <div class="nexus-storage-table-wrap"><table class="nexus-storage-table">
                ...
                {for entries.iter().map(|entry| {
                    ...
                    html! { <tr>
                        <td><strong>{entry.name.clone()}</strong></td><td>{entry.remote.clone()}</td><td>{entry.location.clone()}</td><td><span class="nexus-storage-type">{entry.kind.clone()}</span></td>
                        <td><span class={classes!("nexus-storage-status", status_class)}><i></i>{entry.status.clone()}</span></td><td>{format_bytes(entry.total)}</td><td>{format_bytes(entry.used)}</td><td>{format_bytes(entry.total.saturating_sub(entry.used))}</td>
                        <td><div class="nexus-storage-usage-cell"><span>{format!("{pct:.1}%")}</span><div><i style={format!("width:{pct:.1}%")}></i></div></div></td>
                    </tr> }
                })}
...
pub fn nexus_storage() -> Html {
    ...
    let content = match resources.as_ref() {
        Some(Ok(data)) => {
            ...
            html! { <>
                ...
                {if errors.is_empty() { Html::default() } else { html! { <div class="nexus-storage-warning"><i class="fa fa-exclamation-triangle"></i><span>{format!("Some remotes could not report storage: {}", errors.join(" · "))}</span></div> } }}
    ...
    html! {
        <div class="nexus-storage-page">
            <header class="nexus-storage-header"><div><div class="nexus-storage-eyebrow">{"CAPACITY & DATA PROTECTION"}</div><h1>{"Storage"}</h1><p>{"Unified capacity view across Proxmox VE and Proxmox Backup Server."}</p></div><span class="nexus-storage-live"><i></i>{"Live from PDM"}</span></header>
            {content}
        </div>
    }
}
