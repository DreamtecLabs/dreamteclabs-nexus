...
pub enum Msg {
    Reload,
    Loaded(Result<ResourcesStatus, Error>),
}

pub struct NexusHome {
...
impl NexusHome {
    fn load(&mut self, ctx: &Context<Self>) {
        ...
        self.async_pool.send_future(link, async move {
            Msg::Loaded(http_get("/resources/status", Some(json!({ "max-age": 30 }))).await)
        });
    }
}

impl Component for NexusHome {
    type Message = Msg;
    type Properties = ();
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        let reload = ctx.link().callback(|_| Msg::Reload);
        let body = match (&self.status, &self.error) {
            (Some(status), _) => dashboard(status, reload, self.loading),
            (None, Some(error)) => html! {
                <div class="nexus-empty-state">
                    <div class="nexus-empty-icon"><i class="fa fa-exclamation-triangle"></i></div>
                    <h3>{"Unable to load infrastructure status"}</h3>
                    <p>{error}</p>
                    <button class="nexus-button" onclick={reload}>{"Try again"}</button>
                </div>
            },
            _ => html! {
                <div class="nexus-loading"><i class="fa fa-circle-o-notch fa-spin"></i><span>{"Loading infrastructure overview…"}</span></div>
            },
        };
...
fn dashboard(status: &ResourcesStatus, reload: Callback<MouseEvent>, loading: bool) -> Html {
    let pve_nodes = status.pve_nodes.online + status.pve_nodes.offline + status.pve_nodes.unknown;
    let qemu_total = status.qemu.running + status.qemu.stopped + status.qemu.template + status.qemu.unknown;
    ...
    let memory_percent = percentage_u64(status.pve_memory_stats.used, status.pve_memory_stats.total);
        ...
        <>
            <div class="nexus-page-header">
                <div>
                    ...
                    <div class="nexus-title-row"><h1>{"Dashboard"}</h1><span class="nexus-sync-dot"></span><span class="nexus-muted">{if loading { "Refreshing…" } else { "Live from PDM" }}</span></div>
                    <p>{"Unified view of your Proxmox VE and Backup Server infrastructure."}</p>
                </div>
                <button class="nexus-button nexus-button-secondary" onclick={reload} disabled={loading}>
                    <i class={if loading { "fa fa-refresh fa-spin" } else { "fa fa-refresh" }}></i><span>{"Refresh"}</span>
                </button>
            </div>
            ...
            <div class="nexus-three-column">
                {workload_panel("Virtual Machines", "fa fa-desktop", qemu_total, status.qemu.running, status.qemu.stopped, "Running", "Stopped")}
                {workload_panel("Linux Containers", "fa fa-cube", lxc_total, status.lxc.running, status.lxc.stopped, "Running", "Stopped")}
                {backup_panel(status)}
            </div>
            ...
            <div class="nexus-two-column nexus-bottom-row">
                <section class="nexus-panel">
                    ...
                    <div class="nexus-capacity-list">{capacity_bar("CPU", cpu_percent, "#2563eb")}{capacity_bar("Memory", memory_percent, "#22c55e")}{capacity_bar("Storage", storage_percent, "#f97316")}</div>
                </section>
                <section class="nexus-panel">
...
fn kpi_card(icon: &str, title: &str, value: u64, subtitle: String, healthy: bool, footer: &str) -> Html {
    html! {<section class="nexus-kpi-card"><div class="nexus-kpi-top"><span class="nexus-icon"><i class={icon.to_string()}></i></span><span>{title}</span></div><div class="nexus-kpi-value">{value}</div><div class="nexus-kpi-subtitle">{subtitle}</div><div class={classes!("nexus-kpi-footer", (!healthy).then_some("warning"))}><i class={if healthy { "fa fa-check" } else { "fa fa-exclamation-triangle" }}></i><span>{footer}</span></div></section>}
}

fn alert_card(attention: u64, failed_remotes: u64) -> Html {
    html! {<section class="nexus-kpi-card nexus-alert-card"><div class="nexus-kpi-top"><span class="nexus-icon alert"><i class="fa fa-shield"></i></span><span>{"Active Alerts"}</span></div><div class="nexus-kpi-value">{attention}</div><div class="nexus-kpi-subtitle alert-text">{format!("{} failed remotes · {} infrastructure issues", failed_remotes, attention.saturating_sub(failed_remotes))}</div><div class={classes!("nexus-kpi-footer", (attention > 0).then_some("warning"))}><i class={if attention == 0 { "fa fa-check" } else { "fa fa-exclamation-circle" }}></i><span>{if attention == 0 { "No active infrastructure alerts" } else { "Review infrastructure health" }}</span></div></section>}
}

fn gauge(label: &str, value: f64, detail: String, color: &str) -> Html {
    let safe = value.clamp(0.0, 100.0);
    html! {<div class="nexus-gauge-item"><div class="nexus-gauge" style={format!("background:conic-gradient({color} {safe:.1}%, #edf1f6 {safe:.1}% 100%);")}><div class="nexus-gauge-inner"><strong>{format!("{safe:.0}%")}</strong></div></div><div class="nexus-gauge-label">{label}</div><div class="nexus-gauge-detail">{detail}</div></div>}
}

fn health_row(label: &str, online: u64, issues: u64, detail: String) -> Html {
    html! {<div class="nexus-health-row"><div class="nexus-health-name"><span class="nexus-row-icon"><i class="fa fa-server"></i></span><strong>{label}</strong></div><div><span class="nexus-status-dot"></span><span class="nexus-status-text">{format!("{} online", online)}</span></div><div class={classes!("nexus-issue-count", (issues > 0).then_some("warning"))}>{if issues == 0 { "No issues".to_string() } else { format!("{} issues", issues) }}</div><div class="nexus-muted">{detail}</div></div>}
}

fn workload_panel(title: &str, icon: &str, total: u64, running: u64, stopped: u64, running_label: &str, stopped_label: &str) -> Html {
    let running_pct = if total == 0 { 0.0 } else { running as f64 / total as f64 * 100.0 };
    html! {<section class="nexus-panel nexus-workload-panel"><div class="nexus-panel-header compact"><h2><i class={icon.to_string()}></i>{title}</h2><span class="nexus-link">{"View all →"}</span></div><div class="nexus-workload-body"><div class="nexus-donut" style={format!("background:conic-gradient(#22c55e {running_pct:.1}%, #e5e7eb {running_pct:.1}% 100%);")}><div><strong>{total}</strong><span>{"Total"}</span></div></div><div class="nexus-legend"><div><span class="legend-dot green"></span><span>{running_label}</span><strong>{running}</strong><em>{format!("{:.0}%", running_pct)}</em></div><div><span class="legend-dot gray"></span><span>{stopped_label}</span><strong>{stopped}</strong><em>{format!("{:.0}%", if total == 0 { 0.0 } else { stopped as f64 / total as f64 * 100.0 })}</em></div></div></div></section>}
}

fn backup_panel(status: &ResourcesStatus) -> Html {
    let online = status.pbs_datastores.online;
    let warning = status.pbs_datastores.under_maintenance.unwrap_or_default() + status.pbs_datastores.high_usage.unwrap_or_default();
