...
- **SigNoz**: metrics exploration, dashboards, alert rules and notification routing.
...
The key stays server-side and is never returned to the browser. The initial integration validates connectivity through the SigNoz rules API and exposes only a safe connection summary and rule count to the UI.
...
The data model is designed to expand to HTTP/HTTPS, TCP and SNMP profiles without making SigNoz the inventory source of truth. Alert-rule lifecycle will be managed through the SigNoz API after the exact deployed API schema is validated against the self-hosted instance.
