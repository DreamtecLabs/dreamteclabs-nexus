...
async def test_pdm_lists_and_normalizes_resources_with_token_auth() -> None:
                ...
                "data": [
                    {
                        ...
                        "resources": [
                            {
                                "type": "node",
                                "id": "remote/home-pve/node/pve-01",
                                "node": "pve-01",
                                "status": "online",
                            },
                            {
                            ...
                            {
                                "type": "storage",
                                "id": "remote/home-pve/storage/pve-01/local-lvm",
                                "storage": "local-lvm",
                                "node": "pve-01",
                                "status": "available",
                            },
    ...
    assert captured == {
        "path": "/api2/json/resources/list",
        "authorization": "PDMAPIToken nexus@pam!core:secret",
    }
    assert len(snapshot.resources) == 4
    assert snapshot.resources[0] == InfrastructureResource(
        id="remote/home-pve/guest/104",
        provider="pdm",
        remote="home-pve",
        type="lxc",
    ...
    assert snapshot.remote_errors == (
        InfrastructureRemoteError(remote="offline-pve", detail="remote is unreachable"),
    )
...
class FakeInfrastructureService:
    async def list_resources(self) -> InfrastructureSnapshot:
        return InfrastructureSnapshot(
            ...
            remote_errors=(
                InfrastructureRemoteError(remote="lab-pve", detail="temporarily unavailable"),
            ),
        )
...
def test_infrastructure_api_returns_normalized_snapshot(tmp_path: Path) -> None:
    ...
    assert response.json() == {
        ...
        "remote_errors": [
            {"remote": "lab-pve", "detail": "temporarily unavailable"}
        ],
        "count": 1,
    }
